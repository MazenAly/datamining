...
def get_distance(point1, point2):
    ...
    return spatial.distance.cityblock(np.array(point1), np.array(point2))

def cluster_distance(cluster1, cluster2):
    center1 = np.median(cluster1,0)
    center2 = np.median(cluster2,0)
    ...
    return get_distance(center1, center2)
    ...
    for r in range(1,k):
        ...
        for point in data_clone:
            ...
            for i in range(r):
                ...
                dist = get_distance(point, seeds[i])
                if dist < min_dist:
                    min_dist = dist
...
def kmedians(data, k, method):
    ...
    while True:
        ...
        for point in data:
            ...
            for i in range(k):
                ...
                dist = get_distance(point, seed)
                if dist < min_dist:
                    min_dist = dist
                    current_index = i
            new_clusters[current_index].append(point)
            #cost += np.linalg.norm(point - seeding_points[current_index]) 
            cost += get_distance(point, seeding_points[current_index]) 
         
        print('==============')
        #seeding_points = [np.average(new_clusters[i],0) for i in range(k)]
        seeding_points = [np.median(new_clusters[i],0) for i in range(k)]
    ...
    return new_clusters, cost , costs_array, seeding_points
...
def run_analysis():
    ...
    for m in methods:
        ...
        for k in clusters_no:
            ...
            longest_run = 0
            for i in range(0,5):
                result , cost , costs_array, centers = kmedians(data, k, m)
...
if __name__ == "__main__":
    c1 = load_data_1b("./data1b/C3.txt")
    #run_analysis()
    result,cost, cost_arr, centers = kmedians(c1, int(sys.argv[1]), sys.argv[2])
